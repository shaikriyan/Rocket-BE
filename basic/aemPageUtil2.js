...
async function createPage({ parentPath, template, title, pageName = "" }) {
  ...
  try {
    const response = await axios.post(url, body, { headers: commonHeaders, auth });
...
// Rollout Page
async function rolloutPage({ path, targetPaths = [], reportSchedule = "now" }) {
  const url = `${AEM_DOMAIN}/bin/asynccommand`;

  const body = new URLSearchParams({
    type: "page",
    "msm:targetPath": JSON.stringify(targetPaths),
...
async function copyPage({ srcPath, destParentPath, destName, shallow = "false" }) {
  ...
  try {
    const response = await axios.post(url, form, {
      headers: { ...form.getHeaders(), Accept: "*/*", "X-Requested-With": "XMLHttpRequest" },
      auth
    });
...
async function replicateContent(path, cmd) {
  ...
  try {
    ...
    console.log("✅ Replication successful:", response.data);
    return response.status.toString();
  } catch (error) {
    console.error("❌ Replication error:", error.response?.data || error.message);
...
async function deletePage(pagePath) {
  ...
  const body = new URLSearchParams({ ":operation": "delete" }).toString();
...
async function rolloutByResetting({ srcPath, targetPath }) {
  ...
  try {
    ...
    console.error("❌ Error in rolloutByResetting:", error.response?.data || error.message);
  }
}
